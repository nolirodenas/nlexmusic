...
export default function SongManager({ tracks = [], index = 0, setIndex = () => {}, playlists = [], selected, setSelected = () => {} }) {
  ...
  function nextImmediate() {
    // if multiple playlists -> switch to next playlist and let Dashboard fetch tracks
    if (playlists && playlists.length > 1 && selected) {
      const cur = playlists.findIndex(p => String(p.id) === String(selected));
      const nextPl = playlists[(cur + 1) % playlists.length];
      if (nextPl) {
        setSelected(String(nextPl.id));
        return;
      }
    }
    if (!tracks.length) return;
  ...
  function prevImmediate() {
    if (playlists && playlists.length > 1 && selected) {
      const cur = playlists.findIndex(p => String(p.id) === String(selected));
      const prevPl = playlists[(cur - 1 + playlists.length) % playlists.length];
      if (prevPl) {
        setSelected(String(prevPl.id));
        return;
      }
    }
    if (!tracks.length) return;
  ...
  function handleNext() {
    if (nextClickTimer.current) {
      ...
      const cur = playlists.findIndex(p => String(p.id) === String(selected));
      const nextPl = playlists[(cur + 1) % playlists.length];
      if (nextPl) setSelected(String(nextPl.id));
      return;
    }
  ...
  function handlePrev() {
    if (prevClickTimer.current) {
      ...
      const cur = playlists.findIndex(p => String(p.id) === String(selected));
      const prevPl = playlists[(cur - 1 + playlists.length) % playlists.length];
      if (prevPl) setSelected(String(prevPl.id));
      return;
    }
    ...
    <div className="w-full max-w-xl">
      ...
      {tracks.length > 0 ? (
        <>
          ...
          {showVisualizer && <canvas ref={canvasRef} width={340} height={60} className="w-full h-16 mb-2 bg-black/80 rounded-xl border border-violet-900 shadow" />}
